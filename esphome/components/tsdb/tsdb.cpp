...
// instead of silently invalidating docs/data_logging.md.
...
static_assert(tsdbmath::records_per_block(6) == (TSDB_BLOCK_SIZE - TSDB_BLOCK_HEADER_SIZE) / (4 + 6 * 2),
              "esp_tsdb changed the record geometry");
static_assert(tsdbmath::max_records_for_bytes(3072, 6) == TSDB_CALC_MAX_RECORDS(3072, 6),
...
static_assert(tsdbmath::capacity_for_bytes(384 * 1024, 6) <= TSDB_CALC_MAX_RECORDS(384 * 1024, 6),
...
void TsdbComponent::dump_csv_(uint32_t rows) {
  ...
  ESP_LOGI(TAG, "%s: csv-dump begin (newest %" PRIu32 " records, %s ...)", this->file_.c_str(), rows, header.c_str());
