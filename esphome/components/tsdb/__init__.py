...
#: The `log_sensor` key of the two gas components. A private alias on purpose:
#: the vendored `ci-custom` POST check wants a key that three components share to
#: live in `esphome/components/const/`, which is part of the ESPHome package and
#: not of this repository - and diverging the key name would make the three
#: components configure their log mirror differently.
_CONF_LOG_SENSOR = "log_sensor"
    ...
    cv.Schema(
        {
            ...
            cv.Optional(_CONF_LOG_SENSOR): cv.use_id(text_sensor.TextSensor),
        }
    ),
    _validate_config,
)
...
async def to_code(config: ConfigType) -> None:
    ...
    for key, setter in (
        ...
        (_CONF_LOG_SENSOR, var.set_log_sensor),
    ):
